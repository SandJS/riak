...
class Client {
  ...
  constructor(config) {
     ...
     * @type {Riak.Client}
     */
    this.client = new Riak.Client(new Riak.Cluster(config), (err) => {
      if (err) {
  ...
  client[fn] = function(...args) {
    ...
    return new Promise(function(resolve) {
      ...
      let checkConnectState = function() {
        // We are running
        if (self.cluster._state == Riak.Cluster.State.RUNNING) {
          // Set to connected
      ...
      this.client.cluster.on('stateChange', checkConnectState);
      checkConnectState();
    }).catch(sand.error);
  };
}
